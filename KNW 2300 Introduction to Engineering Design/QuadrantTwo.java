...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public static void main(String[] args) {
		...
		double[] sandbox = new double[]{6.0,8.0};
		double[] volcano_base = new double[]{4.0, 12.0};
		double[] volcano = new double[]{8.0, 16.0};
		RXTXRobot r = new ArduinoUno(); // Create RXTXRobot object
		r.setPort("COM5"); // Set the port to COM3
		int success = 0;
		r.connect();

		r.runTwoPCAMotor(8, -290, 9, -370, 1800); // cross bridge
		while (success != 1) {
		returnAngles(r);
		success = moveToLocation(r, sandbox, beacon_chars, beacon_angles, 2);
		}
		success = 0;
		...
		while (r.getPing(5) > 50) { // 50 cm gives the robot ample time to stop and accounts for error of navigation
			...
			System.out.println("System sensed object was " + r.getPing(5) + "cm away.");
		}
		...
		while (success != 1) {
		returnAngles(r);
		success = moveToLocation(r, volcano_base, beacon_chars, beacon_angles, 2);
		}
		success = 0;
	
		while (success != 1) {
			returnAngles(r);
			success = moveToLocation(r, volcano, beacon_chars, beacon_angles, 2);
		}
		success = 0;
